...
export class SalesCategoriesComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    onConfirm(): void {
        this.currentSalesCategory = this.selectedSalesCategory;
        this.populateForm();
        this.hasSalesCategory = true;
    ...
    populateForm(): void {
        ...
        this.currentSalesCategory.subCategories.forEach(subCategory => {
            (this.salescategoryForm.controls['subCategories'] as FormArray).push(this._formBuilder.group({
                description: [subCategory.description, Validators.required],
    ...
    save(): void {
        ...
        subCategoryGroups.forEach((subCategoryGroup: FormGroup, index) => {
            let descriptionControl = subCategoryGroup.controls['description'];
            let defaultControl = subCategoryGroup.controls['default'];
            let actualSubDefault = 0;
            if (defaultControl.value){
                actualSubDefault = 1;
            }
            subCategoryList.push(new SalesSubCategory({description: descriptionControl.value, default: actualSubDefault}));
        });
        this.currentSalesCategory.subCategories = subCategoryList;

        this.httpService.saveEntity('user', this.currentSalesCategory)
            .subscribe((data: Response) => {
                ...
                this.hasSalesCategory = false;
                this.newSalesCategory = false;
                this.salescategoryForm.markAsPristine();
    ...
    onRemove() {
        let id = this.salescategoryForm.controls['id'];
        this.errors = [];
    ...
    canDeactivate(): Observable<boolean> | Promise<boolean> | boolean {
        if (this.salescategoryForm.dirty) {
